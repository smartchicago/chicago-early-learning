...
def populate_enrollment_info(apps, schema_editor):
    ...
    for loc in Location.objects.all():
        if loc.is_community_based:
            loc.enrollment_en = "<p>Visit this location to begin the enrollment process. Many people find it helpful to make a plan to visit.</p>"
            loc.save()
        if loc.is_cps_based:
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('portal', '0005_location_site_type'),
    ]
